...
class StmExpList {
  Tree.Stm stm;
  LinkedList<Tree.Exp1> exps;
  StmExpList(Tree.Stm s, LinkedList<Tree.Exp1> e) {
    stm=s; exps=e;}
}
...
public class Canon {
 ...
 static StmExpList nopNull = new StmExpList(new Tree.EXP(new Tree.CONST(0)),
                                            new LinkedList<Tree.Exp1>());

 static StmExpList reorder(LinkedList<Tree.Exp1> exps) {
     if (exps.isEmpty()) return nopNull;
     else {
       Tree.Exp1 a = exps.removeFirst();
       if (a instanceof Tree.CALL) {
         Temp.Temp t = new Temp.Temp();
	 Tree.Exp1 e = new Tree.ESEQ(new Tree.MOVE(new Tree.TEMP(t), a),
				    new Tree.TEMP(t));
         exps.addFirst(e);
         return reorder(new LinkedList<Tree.Exp1>(exps));
       } 
       else {
	 ...
	 if (commute(bb.stm, aa.exp)){
	      ...
	      return new StmExpList(seq(aa.stm,bb.stm), 
				    new LinkedList<Tree.Exp1>(bb.exps));
	 }
	 else {
	   Temp.Temp t = new Temp.Temp();
	   ...
	   return new StmExpList( cc, new LinkedList<Tree.Exp1>(bb.exps));
	 }
       }
     }
 }
